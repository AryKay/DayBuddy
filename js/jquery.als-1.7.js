 ...
 */
 ...
 (function($){
	...
	var	methods = {
		...
		init: function(options) {
			this.each(function() {
				...
				/******************************************************
				 * prev and  button inizialization (if present)
				 ******************************************************/
		...
		prev: function(id){
			 ...
			 ***********************************/
			als[id] = data;
			// PREVIOUS EVENT UPDATER arykay
			chart.update(eventData.settings.table[data.current].chartFill);
			console.log(data.current); // for debug purposes
			return (als);
		},
		/**************************************************************
	...
	function nextHandle(e,$obj)
	{
		...
		// NEXT EVENT UPDATER arykay
//		if (data.current == 0) {chart.update(8); }
//		if (data.current == 1) {chart.update(88); }
//		if (data.current == 2) {chart.update(30); }
//		if (data.current == 3) {chart.update(60); }
//		if (data.current == 4) {chart.update(0); }
		chart.update(eventData.settings.table[data.current].chartFill);
		console.log(data.current) // for debug purposes
	}
